...
for num_components in num_components_list:
    for model_name, model_class in [
        ...
        if model_name not in ['Random Forest', 'AdaBoost']:
            # Create a pipeline with StandardScaler and PCA if needed
            if model_name == 'Decision Tree' or model_name == 'Gradient Boosting' or model_name == 'XGBoost':
            ...
            # Define hyperparameters for the current model if needed
            model_param_grid = {}
            if model_name == 'KNN':
                model_param_grid = {
    ...
    if 0 <= r2_train <= 1 and 0 <= r2_val <= 1 and model_grid_search.best_score_ > best_mean_test_score:
        best_mean_test_score = model_grid_search.best_score_
        best_weight = w_new

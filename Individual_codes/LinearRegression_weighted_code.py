...
for num_components in num_components_list:
    ...
    model_param_grid = {
        'model__fit_intercept': [True, False],  # Try with and without intercept
    }
    
    model_pipeline.fit(X_train, y_train)
    
    for fit_intercept in model_param_grid['model__fit_intercept']:
        model_pipeline.set_params(model__fit_intercept=fit_intercept)
        model_pipeline.fit(X_train, y_train)
        ...
        if 0 <= r2_train <= 1 and 0 <= r2_val <= 1:
            ...
            print("Hyperparameters: fit_intercept =", fit_intercept)
